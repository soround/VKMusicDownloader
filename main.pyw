...
class MainWindow(QtWidgets.QMainWindow, mainwindow.Ui_MainWindow):
    ...
    def Downloads(self):
        try:
            if (self.action_7.isChecked()):
                PATH = utils.get_path(self, QFileDialog)

                if PATH == "":
                    QMessageBox.warning(self, "Внимание",
                        "Вы не выбрали путь, поэтому аудиозаписи," + 
                        "будут загружены в директорию с программой")
                    PATH = os.getcwd()
    ...
    def TechInformation(self):
        if self.action_5.isChecked():
            self.tech_info_window = TechInfo(vkapi.HOST_API_PROXY, vkapi.OAUTH_PROXY)
        else:
            self.tech_info_window = TechInfo(vkapi.HOST_API, vkapi.OAUTH)
...
class Downloads_file(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress_range.emit(selected)
